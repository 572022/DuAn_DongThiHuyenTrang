...
namespace WebMULTI.Controllers
{
    public class CartController : Controller
    {
        ...
        public ActionResult ThemGioHang1(int ms, string url, FormCollection f)
        {
            ...
            ViewBag.MaCD = new SelectList(query, "TenColor", "TenColor", f["MaCD"]);
            string color = f["MaCD"];
            List<Cart> lst = LayGioHang();
            Cart sp = lst.Find(n => n.iMaSP == ms);
            //var color = f["radioChecked"];
            // If the radio button is not checked, do not add the item to the cart
            if (sp == null)
            {
                int soLuong;
                if (int.TryParse(f["iSoLuong"], out soLuong))
                { 
                sp = new Cart(ms, soLuong, color);
                    lst.Add(sp);
                }
                else
                {
                    ModelState.AddModelError("iSoLuong", "Giá trị không hợp lệ");
                    return Redirect(url); // hoặc chuyển hướng đến một trang lỗi khác
                }
            }
            else
            {
        ...
        private void GuiEmailXacNhanDonHang(DonHang ddh, KhachHang kh, List<Cart> lstGioHang)
        {
            ...
            body += $"<p>Chào {kh.HoTen},Shop Multi xin thông báo.</p>";
